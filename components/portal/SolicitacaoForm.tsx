...
export default function SolicitacaoForm() {
  ...
  const [submittedTipo, setSubmittedTipo] = useState<'PF' | 'PJ' | null>(null);
  ...
  // Load points for the success screen of PF
  useEffect(() => {
    fetch('/api/pontos-coleta')
      .then((res) => res.json())
      .then((data) => {
        if (Array.isArray(data)) {
          setPontosColeta(data.filter((p) => p.ativo));
        }
      })
      .catch((err) => console.error('Erro ao buscar pontos de coleta:', err));
  }, []);
    ...
    return (
      <div className="text-center py-6 px-2">
        ...
        <h2 className="text-xl font-bold text-gray-900 mb-2">
          {submittedTipo === 'PF' ? 'Descarte Registrado!' : 'Solicitação Recebida!'}
        </h2>

        {submittedTipo === 'PF' ? (
          <div className="space-y-6">
            <p className="text-sm text-gray-500 max-w-lg mx-auto">
              Sua solicitação foi salva em nosso sistema para fins de controle e estatística. Como você solicitou como **Pessoa Física (PF)**, por favor, realize o descarte voluntário em um dos nossos **pontos de coleta físicos** mais próximos de você abaixo:
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left mt-6">
              {pontosColeta.map((ponto) => (
                <div key={ponto.id} className="p-4 rounded-2xl border border-gray-100 bg-gray-50 flex flex-col justify-between h-full">
                  <div>
                    <span className="text-xs font-bold text-green-700 bg-green-100/50 px-2 py-0.5 rounded-md">PONTO DE COLETA</span>
                    <h3 className="font-bold text-gray-800 text-sm mt-1">{ponto.nome}</h3>
                    <p className="text-gray-500 text-xs mt-1">{ponto.endereco}</p>
                    {ponto.horario_funcionamento && (
                      <p className="text-gray-400 text-xs mt-2 font-medium">🕐 {ponto.horario_funcionamento}</p>
                    )}
                  </div>
                  <a
                    href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(ponto.nome + ', ' + ponto.endereco)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full text-center mt-4 bg-white hover:bg-green-50 border border-green-200 text-green-700 font-semibold py-2 rounded-xl text-xs transition-colors flex items-center justify-center gap-1"
                  >
                    📍 Ver no Mapa
                  </a>
                </div>
          ...
          <div className="space-y-4">
            <p className="text-sm text-gray-500 max-w-lg mx-auto leading-relaxed">
              Sua solicitação de coleta empresarial como **Pessoa Jurídica (PJ)** foi recebida e está <span className="font-semibold text-green-700">Em Análise</span>.
            </p>
            <p className="text-sm text-gray-500 max-w-lg mx-auto leading-relaxed">
              Nossa equipe administrativa analisará o volume e o tipo dos materiais eletrônicos descritos para agendar a rota de coleta e alocar um veículo para buscar no seu endereço. Você receberá a confirmação por e-mail ou telefone.
            </p>
          </div>
        )}
        ...
        <button
          ...
          className="mt-8 bg-green-700 hover:bg-green-800 text-white font-semibold py-3 px-8 rounded-xl text-sm transition-colors"
        >
