...
const isResendConfigured = resendApiKey && resendApiKey !== 'COLE_AQUI_SUA_RESEND_API_KEY' && !resendApiKey.startsWith('re_xxxx');
...
export async function POST(request: NextRequest) {
  ...
  // 3. Enviar e-mail com PDF anexo via Resend (Bypass para o MVP)
  ...
  if (resend) {
    try {
      ...
      if (emailError) {
        console.warn('Erro ao enviar e-mail via Resend:', emailError);
      } else {
        emailEnviado = true;
      }
    } catch (sendError) {
      console.warn('Exceção ao enviar e-mail via Resend:', sendError);
    }
  } else {
    console.info('Resend não configurado ou API Key pendente. Pulando envio de e-mail para o MVP.');
  }
